...
# generates all combinations of products rather than using the convex hull fast computation
...
#                 combos.append(these_combos)
        ...
        for i in range(1, self.dim + 2):
            these_combos = list()
            #for combo in itertools.combinations(min_entries, i):
            for combo in itertools.combinations(entries, i):
                these_combos.append(frozenset(combo))
            combos.append(these_combos)
    ...
    @property
    def get_com(self):
        return self.all_combos, self.combos
