...
# generates all combinations of all-solid and all-corrosion products rather than using the convex hull fast computation
...
class PourbaixDiagram(MSONable):
    ...
    def _preprocess_pourbaix_entries(self, entries, nproc=None):
        ...
        for i in range(1, self.dim + 2):
            these_combos = list()
            for combo in itertools.combinations(solid_entries, i):
                these_combos.append(frozenset(combo))
            combos.append(these_combos)
            
        for i in range(1, self.dim + 2):
            these_combos = list()
            for combo in itertools.combinations(ion_entries, i):
                these_combos.append(frozenset(combo))
            combos.append(these_combos)
