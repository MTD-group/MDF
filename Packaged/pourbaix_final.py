# Slightly modified version of the Materials Project pymatgen PourbaixDiagram module
# uses three separate convex hulls for solid products, corrosion products, and all products
# this ensures the most stable all-solid or all-ion combinations are taken into account
...
class PourbaixDiagram(MSONable):
    ...
    def _preprocess_pourbaix_entries(self, entries, nproc=None):
        ...
        min_entries_all, valid_facets_all = self._get_hull_in_nph_nphi_space(entries)
        solid_entries = [entry for entry in entries if entry.phase_type == "Solid"]
        ion_entries = [entry for entry in entries if entry.phase_type == "Ion"]
        min_entries_s, valid_facets_s = self._get_hull_in_nph_nphi_space(solid_entries)
        min_entries_i, valid_facets_i = self._get_hull_in_nph_nphi_space(ion_entries)
#         min_entries = min_entries_all + min_entries_s + min_entries_i
#         valid_facets = valid_facets_all + valid_facets_s + valid_facets_i
#         self.min_entries = min_entries
#         self.valid_facets = valid_facets

        combos = []
        for facet in valid_facets_all:
            for it in range(1, self.dim + 2):
                these_combos = list()
                for combo in itertools.combinations(facet, it):
                    these_entries = [min_entries_all[i] for i in combo]
                    these_combos.append(frozenset(these_entries))
                combos.append(these_combos)
        for facet in valid_facets_s:
            for it in range(1, self.dim + 2):
                these_combos = list()
                for combo in itertools.combinations(facet, it):
                    these_entries = [min_entries_s[i] for i in combo]
                    these_combos.append(frozenset(these_entries))
                combos.append(these_combos)
        for facet in valid_facets_i:
            for it in range(1, self.dim + 2):
                these_combos = list()
                for combo in itertools.combinations(facet, it):
                    these_entries = [min_entries_i[i] for i in combo]
                    these_combos.append(frozenset(these_entries))
                combos.append(these_combos)
